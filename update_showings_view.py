...
cur.execute("DROP VIEW IF EXISTS v_monthly_showings CASCADE")

# 2. Recreate with sale/rent via join to properties
cur.execute("""
    CREATE VIEW v_monthly_showings AS
    SELECT
        date_trunc('month', y.showing_date)::date AS period_start,
        y.agent_id,
        COUNT(*) AS showings,
        COUNT(*) FILTER (WHERE p.sale_rent = 'Πώληση') AS showings_sale,
        COUNT(*) FILTER (WHERE p.sale_rent = 'Ενοικίαση') AS showings_rent
    FROM ypodikseis y
    LEFT JOIN properties p ON y.property_id::text = p.property_id::text
    WHERE y.agent_id IS NOT NULL AND y.showing_date IS NOT NULL
    GROUP BY date_trunc('month', y.showing_date)::date, y.agent_id
""")
print("Recreated v_monthly_showings with sale/rent")

# Verify
cur.execute("""
    SELECT SUM(showings), SUM(showings_sale), SUM(showings_rent)
    FROM v_monthly_showings WHERE period_start = '2026-02-01'
""")
r = cur.fetchone()
print(f"Feb 2026: total={r[0]}, sale={r[1]}, rent={r[2]}")

# 3. Recreate v_combined_metrics
cur.execute("""
...
cur.execute("GRANT SELECT ON v_monthly_showings TO anon, authenticated")
