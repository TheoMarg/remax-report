...
function DeltaBadge({ delta, hasAcc }: { delta: number; hasAcc: boolean }) {
  if (!hasAcc) {
    return <span className="text-[10px] text-[#8A94A0]">CRM only</span>;
  }
  if (delta === 0) {
...
export function KpiCards({ kpis }: Props) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
      {kpis.map((kpi) => {
        const hasAcc = kpi.key !== 'published'; // Δημοσιευμένα has no ACC equivalent
        return (
          <div
            key={kpi.key}
          ...
          >
            ...
            <div className="text-[10px] text-[#8A94A0] mb-1">CRM</div>
            {hasAcc && (
