...
export function FunnelNarrative({ rows }: Props) {
  ...
  const top = rows[0];
  insights.push(`Η κατηγορία με τις περισσότερες καταγραφές είναι "${top.subcategory}" (${top.registrations.toLocaleString('el-GR')}).`);
  ...
  if (withConv.length > 0) {
    ...
    insights.push(`Καλύτερο ποσοστό μετατροπής: "${best.subcategory}" (${best.convPct}%).`);
  }
  ...
  if (totalReg > 0) {
    ...
    insights.push(`Συνολικά: ${totalReg.toLocaleString('el-GR')} καταγραφές → ${totalClose.toLocaleString('el-GR')} κλεισίματα (${overallConv}%).`);
  }
