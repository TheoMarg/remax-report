...
-- New properties registered per agent per month (by registration_date = "Καταχωρήθηκε" event)
...
-- New exclusive contracts signed per agent per month.
-- Gsheet = source of truth (Jan 15 2026+). Excel for historical.
-- Dedup by property_code, gsheet preferred.
CREATE VIEW v_monthly_exclusives
WITH (security_invoker = true) AS
WITH deduped AS (
    -- Gsheet records (authoritative)
    ...
    UNION ALL
    -- Excel-only (historical, signed before gsheet cutoff)
    SELECT property_code, agent_id, sign_date, property_id
    FROM exclusives
    WHERE agent_id IS NOT NULL AND sign_date IS NOT NULL
      AND source LIKE 'excel%'
      AND sign_date < DATE '2026-01-15'
      AND property_code NOT IN (
          SELECT property_code FROM exclusives WHERE source LIKE 'gsheet%'
      )
)
SELECT
    date_trunc('month', e.sign_date)::date       AS period_start,
    e.agent_id,
    COUNT(DISTINCT e.property_code)              AS exclusives,
    COUNT(DISTINCT e.property_code) FILTER (WHERE p.subcategory IN (
...
-- ══ v_monthly_closings ══
-- Closings per agent per month (from status_changes: "Έκλεισε από εμάς")
...
-- Uses a spine of all (period_start, agent_id) pairs from every sub-view
-- so agents with activity in ANY metric are included.
...
LEFT JOIN team_members tm
    ON s.agent_id = tm.agent_id
LEFT JOIN teams t
    ON tm.team_id = t.team_id
LEFT JOIN v_monthly_crm_metrics c
    ON s.period_start = c.period_start AND s.agent_id = c.agent_id
LEFT JOIN v_monthly_acc_metrics a
    ON s.period_start = a.period_start AND s.agent_id = a.agent_id
LEFT JOIN v_monthly_registrations reg
    ON s.period_start = reg.period_start AND s.agent_id = reg.agent_id
LEFT JOIN v_monthly_exclusives ex
    ON s.period_start = ex.period_start AND s.agent_id = ex.agent_id
LEFT JOIN v_monthly_published pub
    ON s.period_start = pub.period_start AND s.agent_id = pub.agent_id
LEFT JOIN v_monthly_showings sh
    ON s.period_start = sh.period_start AND s.agent_id = sh.agent_id
LEFT JOIN v_monthly_closings cl
    ON s.period_start = cl.period_start AND s.agent_id = cl.agent_id
LEFT JOIN v_monthly_billing bi
    ON s.period_start = bi.period_start AND s.agent_id = bi.agent_id;
...
-- Parsed withdrawal reasons per agent per month
-- Includes both deactivation AND closing_ours (per BROKER_REPORT_RULES §8)
